...
{
    public partial class T2UserEdit : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sql = "update T2_MapUser set UserName='" + txtUser.Text.Trim() + "', Pwd='" + txtPwd.Text.Trim() + "' where ID=1";
                SqlCommand cmd = new SqlCommand(sql, conn);
                int ret = cmd.ExecuteNonQuery();
                if (ret == 1)
                {
                    Page.ClientScript.RegisterStartupScript(ClientScript.GetType(), "", "<script>alert('恭喜重置用户成功！');self.location='T2Login.aspx';</script>");
                }
            }
