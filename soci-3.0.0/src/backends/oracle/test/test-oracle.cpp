...
#define DB_PARAMTYPE_IN		1	//输入参数
#define DB_PARAMTYPE_OUT	2	//输出参数
...
// 		statement st2(sql);
// 		st2.exchange(use(out, "output"));
// 		st2.exchange(use(in, "input"));
// 		st2.prepare("begin soci_test(:output, :input); end;");
// 		st2.define_and_bind();
// 		st2.execute(1);

// 		statement st = (sql.prepare << "begin soci_test(:output, :input); end;");
// 		st.exchange(use(out, "output"));
// 		st.exchange(use(in, "input"));
...
// 			use(r, "output"));
// 		stInner.exchange(into(name));
// 		stInner.exchange(into(name2));
// 		stInner.exchange(into(name3));
// 		proc.execute(1);
        
		statement st = (sql.prepare << "begin soci_test(:output, :input); end;", use(out, "output"), use(in, "input"));
        st.execute(1);
        assert(out == in);
...
//             << " connectstring\n"
//             << "example: " << argv[0]
//             << " \'service=192.168.27.120:1521/orcl user=sa password=123456\'\n";
//         std::exit(1);
//     }

	connectString = "service=192.168.27.120:1521/orcl user=sa password=123456";

// 	IDBConnectionFactory* factory = NULL;
// 	IDBConnection* conn = NULL;
// 	IDBParameters* param = NULL;
// 	IDBRecordset* rs = NULL;
// 	factory = new COracleDBConnectionFactory();
// 	conn = factory->CreateOracleConnection("192.168.27.120", "orcl","sa", "123456");
// 	param = conn->CreateParameters();
// 	int key1 = 0;
// 	int key2 = 0;
// 	param->AddParamInt(L"keyIn", key1, DB_PARAMTYPE_IN);
// 	param->AddParamInt(L"KeyOut", key2, DB_PARAMTYPE_OUT);
// 	conn->ExecProc(L"SP_TestRS", param, &rs);
// 	param->GetInt(L"KeyOut", &key2);
// 	while(1)
// 	{
// 		rs->GetInt(L"hello", &key1);
// 		rs->MoveNext();
// 	}
...
// 	try
// 	{
// 		session sql(oracle, "service=192.168.27.120:1521/orcl user=sa password=123456");
// 
// 		int count;
// 		sql << "select count(*) from pqrstest", into(count);
// 
...
// 			use(stInner, "output"));
// 		stInner.exchange(into(name));
// 		//stInner.exchange(into(strTestObj));
// 		stInner.exchange(into(name2));
// 		stInner.exchange(into(name3));
// 		proc.execute(1);
// 		
...
// 			string phone;
// 			indicator ind;
// 			sql << "select string2 from pqrstest where string2 = :name",
